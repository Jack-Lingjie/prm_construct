...
def get_split_prompt(task, x, ys, step):
    """get split prompt
    """
...
def get_filter_results(task, nodes, input_prompts, ys, new_ys, max_samples, n_generate_sample, step, idx):
    ...
    for i, node in enumerate(nodes):
        ...
        for j in range(1, n_generate_sample):
            idx += 1
            temp_node = Node(qid=node.qid, idx=idx, x=ys[i], y=new_ys[i + j], step=step, parent=node.idx)
...
def bsf_solve(args, task, qid, model, to_print=True):
    ...
    for step in range(task.steps):
        ...
        input_prompts = get_split_prompt(task, x, ys, step)
        # get sample y
        # NOTE y need to append to the input x
        new_ys = get_samples(task, model, input_prompts, ys, args.n_generate_sample)
        # get new results
